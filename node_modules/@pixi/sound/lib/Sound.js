...
const _Sound = class {
   ...
   */
  preferUrl(urls) {
    const [file] = urls.map((url) => ({ url, ext: pixi_js.path.extname(url).slice(1) })).filter(({ ext }) => supported.supported[ext]).sort((a, b) => supported.extensions.indexOf(a.ext) - supported.extensions.indexOf(b.ext));
    if (!file) {
